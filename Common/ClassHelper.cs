...
namespace Ajax.Common
{
	...
	public class ClassHelper
	{
		...
		/// <summary>
		/// 根据类的实例设置类的属性值，传入属性/值字典
		/// </summary>
		/// <param name="classInstance">将要设置的类的实例。</param>
		/// <param name="collection">属性/值字典</param>
		public static void SetPropertyValue(object classInstance, Dictionary<string, string> collection)
		{
			foreach (KeyValuePair<string, string> item in collection)
			{
				classInstance.GetType().InvokeMember(item.Key, BindingFlags.SetProperty,
												 null, classInstance, new object[] { Convert.ChangeType(item.Value, item.Value.GetType()) });
			}
		}
		...
		public class CustPropertyInfo
		{
			...
			/// <summary>
			/// 根据属性名称构造实例。属性类型默认取String
			/// </summary>
			/// <param name="propertyName">属性名称。</param>
			public CustPropertyInfo(string propertyName)
			{
