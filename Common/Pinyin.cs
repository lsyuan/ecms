...
namespace Ajax.Common
{
	...
	{
		#region 拼音字典
		static string allhzpy = @"3007 (ling2)
4E00 (yi1)
4E01 (ding1,zheng1)
4E02 (kao3)
4E03 (qi1)
...
9FA5 (yue4)";
		...
		static Dictionary<int, char> hz2py = new Dictionary<int, char>();
		static Pinyin()
		{
			string[] lines = allhzpy.Split('\n');
			foreach (string line in lines)
			{
				string[] h2y = line.Split(' ');
				hz2py.Add(Convert.ToInt32(h2y[0], 16), h2y[1][1]);
			}
		}
		/// <summary>
		/// 获取汉字拼音（首字母拼音，非全拼）
		...
		{
			if (string.IsNullOrEmpty(hanzi)) return hanzi;
			StringBuilder sb = new StringBuilder(32);
			char c;
			foreach (char ch in hanzi)
			{
				if (hz2py.TryGetValue(ch, out c)) sb.Append(c);
			}
			if (sb.Length == 0)
			{
				sb.Append(hanzi);
